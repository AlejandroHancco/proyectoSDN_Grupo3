...
# MySQL:
def getUserDb(username, db_config,aux):
    try:
...
def updateUser(username, nuevos_valores):
    conexion = None
    try:
...
def createAuthFlows(usuario):
    ...
    # Update del usuario:
    updateUser(usuario.username, {"ip": ip_usuario, "sw_id": sw_id_usuario, "sw_port": sw_port_usuario, "mac": mac_usuario, "numrules": numrules})

    return regla_usuario

# Session:
def createSession(username):
    updateUser(username,{"session": generate_session_id(), "time_stamp": get_date()})
